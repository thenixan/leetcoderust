...
impl Iteration {
    fn new(next: String, value: u16) -> Self { Iteration { next, value } }
}
...
pub fn run() {
    ...
    loop {
        if values.contains_key("a") {
            println!("Result: {}", values.get("a").unwrap());
            break;
        }
        for s in commands.iter().filter(|c| { c.inputs().iter().all(|f| values.contains_key(&f.to_string())) }) {
            ...
            values.insert(s.output().to_string(), s.run(args));
            println!("{:?}", s)
        }
    }
}
...
impl Identified for Vec<Iteration> {
    fn identificators(&self) -> Vec<&str> {
        self.into_iter().map(|i| i.next.as_str()).collect()
    }
